...
function saveRequest(data){
  ...
  sessionStorage.setItem("rafin_request_return", data.service === "marketing" ? "marketing.html" : "web-development.html");
}
...
function initMarketingForm(formId){
  ...
  form.addEventListener("submit", e => {
    ...
    if(data.brand_type !== "Other") delete data.other_category;
    saveRequest(data);
    location.href = "review.html";
  });
}
...
function initReviewPage(){
  ...
  if(!raw){
    box.innerHTML = "<div class='reviewRow'><div class='reviewValue'>No request data was found.</div></div>";
  ...
  box.innerHTML = Object.entries(data)
    ...
    .map(([key,value]) => `
      <div class="reviewRow">
        ...
        <div class="reviewValue">${escapeHtml(key === "service" ? (value === "marketing" ? "Marketing" : "Website Development") : value)}</div>
      </div>
    `).join("");

  back.onclick = () => {
    location.href = sessionStorage.getItem("rafin_request_return") || "services.html";
  };
  ...
  confirm.onclick = async () => {
    ...
    try{
      ...
      if(!response.ok){
        ...
        throw new Error(detail || "Unable to save request.");
      }
      ...
      console.error(err);
      error.textContent = "The request could not be saved. Please check the Supabase table and Row Level Security policy.";
      error.hidden = false;
...
function initChat(){
  ...
  const addMessage = (text, type) => {
    ...
    el.className = type === "user" ? "userMessage" : "assistantMessage";
    el.textContent = text;
  ...
  form.addEventListener("submit", async e => {
    ...
    status.textContent = "Generating a response...";
